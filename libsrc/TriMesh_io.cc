...
static bool read_smf(FILE *f, TriMesh *mesh);
...
static bool write_smf(TriMesh *mesh, FILE *f);
...
static bool read_smf(FILE *f, TriMesh *mesh)
{
	...
	while (fgets(buf, 1023, f) != NULL)
	{
		if (begins_with(buf, "v "))
		{
			...
			mesh->vertices.push_back(point(v1, v2, v3));
		}
		else if (begins_with(buf, "f "))
		{
			sscanf(buf, "%c %d %d %d", &c, &p1, &p2, &p3);
			mesh->faces.push_back(TriMesh::Face(p1, p2, p3));
		}
	}
	return true;
}
...
bool TriMesh::write(const char *filename)
{
	...
	enum { PLY_ASCII, PLY_BINARY_BE, PLY_BINARY_LE,
	       RAY, OBJ, OFF, SM, STL, CC, DAE, SMF } filetype;
	...
	if (ends_with(filename, ".ply"))
		...
		filetype = DAE;
	else if (ends_with(filename, ".smf"))
		filetype = SMF;
	...
	while (1) {
		if (begins_with(filename, "norm:")) {
			...
			filename += 4;
			filetype = DAE;
		} else if (begins_with(filename, "smf:")) {
			filename += 4;
			filetype = SMF;
		} else {
			break;
